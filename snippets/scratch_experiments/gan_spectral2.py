...
##################################################
# Conditional Generator (BatchNorm Removed)
##################################################
class ConditionalGenerator(nn.Module):
    def __init__(self, encoding_dim, latent_dim, channels=3, feature_map_base=64, image_size=64):
        ...
        # Transposed conv stack without BatchNorm
...
##################################################
# Conditional Discriminator with Intermediate Layer
##################################################
class ConditionalDiscriminator(nn.Module):
    def __init__(self, encoding_dim, channels=3, feature_map_base=64, image_size=64):
        ...
        self.fc1 = spectral_norm(nn.Linear(in_features + encoding_dim, 4096))
        self.leaky_relu = nn.LeakyReLU(0.2, inplace=True)
        self.fc2 = spectral_norm(nn.Linear(4096, 1))
...
def trainFeaturesGAN(
        ...
        channels=3,
        latent_dim=100,
        batch_size=16,          # Increased to 16
        epochs=5,               # Reduced epochs for testing
        lr=2e-4,                # Standard GAN learning rate
    ...
    for epoch in range(epochs):
        ...
        for i, (enc_batch, real_imgs) in enumerate(dataloader):
            ...
            for _ in range(n_critic):
                ...
                # Check for NaNs in D_real and D_fake
                if torch.isnan(D_real).any() or torch.isnan(D_fake).any():
                    print(f"NaN detected in Discriminator outputs at Epoch {epoch + 1}, Batch {i}")
                    return
            ...
            with torch.autograd.detect_anomaly():
                ...
                D_fake_for_G = netD(fake_imgs2, enc_batch)  # (batch,1)
...
state_dict = torch.load("generator_spectral_hinge.pth", map_location=device)
