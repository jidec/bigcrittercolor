...
def test_infermasks():
    inferMasks(img_ids=None,skip_existing=False,data_folder="D:/GitProjects/bigcrittercolor/tests/dummy_bcc_folder",
               text_prompt="dragonfly", archetype="prompt1", erode_kernel_size=3, show=True, show_indv=True)
#test_infermasks()

#text_prompt="dragonfly . wing ."
#archetype="remove_prompt2_from1"

def test_infermasksindv():
    ids = ["INAT-1828591-1"]
    inferMasks(img_ids=ids, skip_existing=False,
               text_prompt="dragonfly . wing .", archetype="remove_prompt2_from1",
