...
def region_growing(image, seeds, initial_threshold=10):
    ...
    # Convert the entire image to LAB once for efficiency
        ...
        while stack:
            ...
            for nx, ny in get_neighbors(px, py):
                if not processed[ny, nx]:
                    neighbor_color = lab_image[ny, nx]
                    if not np.array_equal(image[ny, nx], [0, 0, 0]):  # Exclude black background
...
from bigcrittercolor.helpers import _runtime
# Perform region growing
regions = region_growing(image, seeds, initial_threshold=20)
#regions = _runtime(region_growing,image,seeds,20)
