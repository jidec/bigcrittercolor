...
from bigcrittercolor.helpers import _showImages, _bprint, _getIDsInFolder, _loadImg, showBCCImages

# filter masks using simple metrics, then cluster using features from a pretrained CNN feature extractor
def clusterMasksExtractSegs(img_ids, filter_hw_ratio_minmax = (0, 100), filter_prop_img_minmax =(0, 0.15),
    ...
    img_locs = [data_folder + "/masks/" + id + "_mask.jpg" for id in img_ids]
    ...
    for index, img_loc in enumerate(img_locs):
        ...
        mask = _loadImg(img_loc, print=print_details)

        if mask is None: continue

        if filter_prop_img_minmax is not None:

            white_pixel_count = cv2.countNonZero(mask) # get number of white pixels as nonzeros
            ...
            if white_area_percent < filter_prop_img_minmax[0]:
                ...
                failed_prop_min_ids.append(img_ids[index])
                continue
            if white_area_percent > filter_prop_img_minmax[1]:
                ...
                failed_prop_max_ids.append(img_ids[index])
                continue
        ...
        if filter_hw_ratio_minmax is not None:
            ...
            mask_vert = _verticalizeImg(mask)
