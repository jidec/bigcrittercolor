...
def clusterColorsToPatterns(img_ids=None, preclustered = False, group_cluster_records_col = None, group_cluster_raw_ids = False,
    ...
    for cpd in contour_or_pixel_data:
        ...
        if write_subfolder != "":
            if not os.path.exists(data_folder + "/data/patterns/" + write_subfolder):
                os.mkdir(data_folder + "/data/patterns/" + write_subfolder)
            write_subfolder = write_subfolder + "/"
        write_target = data_folder + "/data/patterns/" + write_subfolder + populated_ids[i] + "_pattern.png"
