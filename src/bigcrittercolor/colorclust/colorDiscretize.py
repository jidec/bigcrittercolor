...
def colorDiscretize(image_ids, preclustered = False, group_cluster_records_col = None, group_cluster_raw_ids = False,
    ...
    for id in image_ids:
        if not preclustered:
            img = cv2.imread(data_folder + "/data/segments/" + id + "_segment.png",cv2.IMREAD_UNCHANGED)
        else:
        ...
        showImages(show_indv,[img],"Segment")
        ...
        if bilat_blur:
            ...
            img[:, :, 3] = alpha
            showImages(show_indv,[img],['Blurred Image'])
    ...
    if group_cluster_records_col is not None:
        ...
        for g in unique_groups:
            ...
            clustered_values = getClusterCentroids(group_contour_means,cluster_model,nclusters,cluster_eps,cluster_min_samples,scale,use_positions,downweight_axis,upweight_axis,preclustered,nclust_metric,img,colorspace,show)
        ...
        clustered_values = getClusterCentroids(group_means, group_coords, cluster_model, nclusters, cluster_eps, cluster_min_samples,scale,use_positions,downweight_axis,upweight_axis,preclustered,nclust_metric,img,colorspace,show)
    ...
    # if not grouping, just cluster at the level of individual images
        ...
        for index, cpd in enumerate(contour_or_pixel_data):
            ...
            clustered_values = getClusterCentroids(cpd[1], cpd[0], cluster_model, nclusters, cluster_eps, cluster_min_samples,scale,use_positions,downweight_axis,upweight_axis,preclustered,nclust_metric,img,colorspace,show)
